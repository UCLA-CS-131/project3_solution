  ...
  def _cache_function_parameters_and_return_type(self, tokenized_program):
    ...
    for line_num, line in enumerate(tokenized_program):
      ...
      if line and line[0] == InterpreterBase.LAMBDA_DEF:
        # format:  lambda p1:t1 p2:t2 p3:t3 return_type
        # let's identify lambda functions by: "lambda:{line_num}"
        func_name = FunctionManager.create_lambda_name(line_num)
        params = [self._to_tuple(formal) for formal in line[1:-1]]
        func_info = FuncInfo(params, line_num + 1)  # function starts executing on line after funcdef
        self.func_cache[func_name] = func_info
        return_type_stack.append(line[-1]) # update our return type to the new lambda's return type
      # v3

      if line and (line[0] == InterpreterBase.ENDFUNC_DEF or
                   line[0] == InterpreterBase.ENDLAMBDA_DEF):  #  V3 for lambda
